...
{
    if (Val)
        HAL_GPIO_WritePin(SPIx_CS_PIN_GPIO_Port, SPIx_CS_PIN_Pin, GPIO_PIN_SET);
    else
        HAL_GPIO_WritePin(SPIx_CS_PIN_GPIO_Port, SPIx_CS_PIN_Pin, GPIO_PIN_RESET);
}
void PS2_CLK(uint8_t Val)
{
    if (Val)
        HAL_GPIO_WritePin(SPIx_SCK_PIN_GPIO_Port, SPIx_SCK_PIN_Pin, GPIO_PIN_SET);
    else
        HAL_GPIO_WritePin(SPIx_SCK_PIN_GPIO_Port, SPIx_SCK_PIN_Pin, GPIO_PIN_RESET);
}
void PS2_DO(uint8_t Val)
{
    if (Val)
        HAL_GPIO_WritePin(SPIx_MOSI_PIN_GPIO_Port, SPIx_MOSI_PIN_Pin, GPIO_PIN_SET);
    else
        HAL_GPIO_WritePin(SPIx_MOSI_PIN_GPIO_Port, SPIx_MOSI_PIN_Pin, GPIO_PIN_RESET);
}
uint8_t PS2_Read_DI()
{
    return HAL_GPIO_ReadPin(SPIx_MISO_PIN_GPIO_Port, SPIx_MISO_PIN_Pin);
}
void PS2_Delay()
{
